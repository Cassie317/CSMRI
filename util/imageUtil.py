...
def kspace_subsampling_pytorch(srcImg,mask):
    ...
    # label = label[0]
    # label[0] = standardization(label[0])*255
    # label = Image.fromarray(label[0])
    # label = label.convert('L')  # 这样才能转为灰度图，如果是彩色图则改L为‘RGB’
    # label.save("train/label4__img2.png")
    # c传进来正常img图
...
def png2kspace(png):
    y = png
    xGT_c =  y.permute(0,2,3,1)
    xGT_f = torch.ifft(xGT_c,2, normalized=True)
    return xGT_f
...
def imgFromSubF_pytorch(subF,returnComplex=False):
    ...
    # img_back = np.abs(subIm.cpu().data.numpy()[0])
    # img_back[img_back < 0] = 0
    # new_arr = standardization(img_back)*255
    # label = Image.fromarray(new_arr)
    # label = label.convert('L')  # 这样才能转为灰度图，如果是彩色图则改L为‘RGB’
    # label.save("resoutput/label4__img3.png")

    if(len(subIm.shape)==4):
        subIm = subIm.permute(0,3,1,2)
        # subIm = [[subIm[0]],[subIm[3]],[subIm[1]],[subIm[2]]]
    else:
        subIm = subIm.permute(0,4,1,2,3)

    # subIm = standardization(subIm)
    if(returnComplex):
        label = subIm.cpu().data.numpy()
        label = label[0,0:1].astype('float64')
        label[0]=standardization(label[0])*255
        # label[0] *= 255  # 变换为0-255的灰度值
        label = Image.fromarray(label[0])
        label = label.convert('L')  # 这样才能转为灰度图，如果是彩色图则改L为‘RGB’
        label.save("resoutput/label4__img2.png")
        return subIm
    else:
